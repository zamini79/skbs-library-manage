...
### 대여 정책
- 기본 대여 기간: **14일**
- 반납일이 **주말·공휴일이면 다음 영업일로 이월** (연휴 연속 처리, 실질 대여기간이 14일보다 길어질 수 있음)
- 공휴일은 `holidays` 테이블 기준. 공공데이터포털 특일정보 API를 **매월 1일 동기화**
  (별도 크론 없이 일일 크론 `update-overdue` 가 수행 — Hobby 플랜 크론 제한 때문).
  자체 휴무일은 `source='manual'` 로 직접 INSERT 하면 동기화가 덮어쓰지 않음
